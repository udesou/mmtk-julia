...
use crate::julia_scanning::{process_edge, read_stack, get_stack_addr};
use crate::julia_types::{mmtk_jl_task_t, UINT32_MAX, mmtk_jl_gcframe_t};
...
impl Scanning<JuliaVM> for VMScanning {
    fn scan_roots_in_mutator_thread(
        ...
        let mut pinning_edge_buffer = EdgeBuffer {buffer: vec![]}; // roots from the shadow stack that we know that do not need to be tpinned
        ...
        let mut root_scan_task = |task: *const mmtk__jl_task_t, task_is_root: bool| {
            if !task.is_null() {
                unsafe {
                    mmtk_scan_gcstack_root(task, &mut edge_buffer, &mut pinning_edge_buffer);
                }
                if task_is_root {
...
pub unsafe fn mmtk_scan_gcstack_root<EV: EdgeVisitor<JuliaVMEdge>>(
